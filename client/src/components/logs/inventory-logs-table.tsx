...
import { useInventoryLogsStore } from "@/store/inventoryLogs"

const actionTypes = ["All Actions", "ADD", "REMOVE", "TRANSFER", "RETURN", "SALE"]
...
export default function InventoryLogsTable() {
  const {
    ...
    setSearchTerm,
    setDateFilter,
  } = useInventoryLogsStore()
  const [isFilterOpen, setIsFilterOpen] = useState(false)
  const [selectedAction, setSelectedAction] = useState("All Actions")

  useEffect(() => {
    fetchLogs(searchTerm, dateFilter)
    // eslint-disable-next-line
  }, [searchTerm, dateFilter])

  const filteredLogs = selectedAction === "All Actions"
    ? logs
    : logs.filter((log) => log.type === selectedAction)
  ...
  const getActionIcon = (type: string) => {
    switch (type) {
      ...
      case "SALE":
        return <ArrowDown className="w-4 h-4 text-purple-400" />
      default:
          ...
          <table className="w-full">
            <thead className="bg-background">
              <tr>
                ...
                <th className="px-6 py-4 text-left text-xs font-medium text-muted uppercase tracking-wider">Date & Time</th>
                  ...
                  <tr key={log.id} className="hover:bg-background transition-colors">
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div>
                        ...
                        <div className="text-sm text-muted">{log.productSku}</div>
                      </div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center gap-2">
                        {getActionIcon(log.type)}
                        <span className={`inline-flex px-2 py-1 text-xs font-medium rounded-full ${getActionColor(log.type)}`}>
                          {log.type}
                        </span>
                      </div>
                    </td>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-muted">{log.userName}</td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm text-primary">{date}</div>
                      <div className="text-xs text-muted">{time}</div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-muted">{log.categoryName}</td>
                  </tr>
                )
              })}
            </tbody>
                ...
                <div key={log.id} className="p-4 hover:bg-background transition-colors">
                  ...
                  <div className="flex items-start justify-between mb-3">
                    <div className="flex-1 min-w-0">
                      <h3 className="font-medium text-primary truncate">{log.productName}</h3>
                      <p className="text-sm text-muted">{log.productSku}</p>
                    </div>
                  ...
                  <div className="grid grid-cols-2 gap-3 text-sm mb-3">
                    ...
                    <div>
                      ...
                      <span className="ml-2 text-primary">{log.userName}</span>
                    </div>
                    <div>
                      <span className="text-muted">Date:</span>
                      <span className="ml-2 text-primary">{date}</span>
                    </div>
                    <div>
                      <span className="text-muted">Time:</span>
                      <span className="ml-2 text-primary">{time}</span>
                    </div>
                  </div>
                  ...
                  <div className="pt-3 border-t border-custom">
                    ...
                    <p className="text-sm text-primary mt-1">{log.categoryName}</p>
                  </div>
                </div>
              )
            })}
                ...
                return (
                  <tr key={log.id} className="hover:bg-background transition-colors">
                    <td className="px-4 py-3">
                      <div>
                        <div className="font-medium text-primary text-sm">{log.productName}</div>
                        <div className="text-xs text-muted">{log.productSku}</div>
                      </div>
                    </td>
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                    ...
                    <td className="px-4 py-3 text-sm text-muted">{log.userName}</td>
                    <td className="px-4 py-3">
                      <div className="text-sm text-primary">{date}</div>
                      <div className="text-xs text-muted">{time}</div>
                    </td>
                    <td className="px-4 py-3 text-sm text-muted max-w-32 truncate">{log.categoryName}</td>
                  </tr>
                )
              })}
            </tbody>
